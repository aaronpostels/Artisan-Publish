...
export class Renderer2D {
  constructor(renderer) {
    ...
    this.polyData = new Float32Array(65536 * 6);
  ...
  generateSDFAtlas() {
    ...
    for (let ay = 0; ay < atlasSize; ay++) {
        for (let ax = 0; ax < atlasSize; ax++) {
            ...
            const avgDistAtlasPx = avgDistHiRes / SS;
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (i === 0) {
        ...
        if (Math.abs(cross) < 1e-6) {
          ...
          mx /= mlen; my /= mlen;
          const cosHalf = Math.max(mx * nAx + my * nAy, 0.15);
